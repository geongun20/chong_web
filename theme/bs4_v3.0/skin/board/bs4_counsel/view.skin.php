...
<?php
	if (!$is_admin) {
		$hp = substr(hyphen_hp_number($view['wr_2']),0,-7)."**-****";
		$secret_name = preg_replace('/.(?=.$)/u','*',$view['wr_name']);
	} else {
		$hp = $view['wr_2'];
		$secret_name = $view['wr_name'];
	}
?>
...
<article id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <section id="bo_v_info">
        <h2>페이지 정보</h2>
        작성자 <strong><?php echo $secret_name ?><?php if ($is_ip_view) { echo "&nbsp;($ip)"; } ?></strong>
    ...
    <div id="bo_v_top">
        ...
        <ul class="bo_v_nb">
            <?php if ($prev_href) { ?><li><a href="<?php echo $prev_href ?>" class="btn btn-raised btn-dark btn-sm">이전글</a></li><?php } ?>
            <?php if ($next_href) { ?><li><a href="<?php echo $next_href ?>" class="btn btn-raised btn-dark btn-sm">다음글</a></li><?php } ?>
        </ul>
        <?php } ?>

        <ul class="bo_v_com">
	        <?php if ($update_href) { ?><li><a href="<?php echo $update_href ?>" class="btn btn-raised btn-secondary btn-sm">수정</a></li><?php } ?>
            <?php if ($delete_href) { ?><li><a href="<?php echo $delete_href ?>" onclick="del(this.href); return false;" class="btn btn-raised btn-secondary btn-sm">삭제</a></li><?php } ?>
            ...
            <li><a href="<?php echo $list_href ?>" class="btn btn-raised btn-info btn-sm">목록</a></li>
        </ul>
        <?php
    ...
    <section id="bo_v_atc">
        ...
        <div class="view-content">
			<?php if ($view['is_notice']) {
				...
				<div class="row">
					<div class="col-6">
						<?="<span class='empha'>이 름: </span>"; ?> <?=$secret_name;?>
					</div>
					<div class="col-6">
						<?="<span class='empha'>전화번호: </span>"; ?> <?=$hp;?>
					</div>
				</div>
				<hr>
				<div class="row">
					<div class="col-12">
						<?="<span class='empha'>상담 내용</span>"; ?>
					</div>
				</div>
				<div class="row mt-2">
					<div class="col-12">
