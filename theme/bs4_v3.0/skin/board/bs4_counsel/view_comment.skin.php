...
//	관리자 열람 시 진행중으로 변경
if($is_admin && !$view['wr_1'])
	sql_fetch(" update $write_table set wr_1='확인중' where wr_id='{$view['wr_id']}' ");
?>
...
<section id="bo_vc">
    ...
    <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0"<?php } ?>>
        ...
        <footer1>
            <ul class="bo_vc_act">
                <?php if ($list[$i]['is_reply']) { ?><li><a href="<?php echo $c_reply_href;  ?>" onclick="comment_box('<?php echo $comment_id ?>', 'c'); return false;"><button class="btn btn-raised btn-secondary btn-sm">답변</button></a></li><?php } ?>
                <?php if ($list[$i]['is_edit']) { ?><li><a href="<?php echo $c_edit_href;  ?>" onclick="comment_box('<?php echo $comment_id ?>', 'cu'); return false;"><button class="btn btn-raised btn-secondary btn-sm">수정</button></a></li><?php } ?>
                <?php if ($list[$i]['is_del'])  { ?><li><a href="<?php echo $list[$i]['del_link'];  ?>" onclick="return comment_delete();"><button class="btn btn-raised btn-secondary btn-sm">삭제</button></a></li><?php } ?>
            </ul>
        </footer1>
        <?php } ?>
    </article>
	...
	<?php if ($is_guest) { ?>
		<div class="row">
			<div class="form-group label-floating col-6">
				...
				<input type="text" class="form-control" name="wr_name" id="wr_name" value="<?php echo get_cookie("ck_sns_name"); ?>" required>
			</div>
  ...
  <div class="form-check">
    ...
    <?php } ?>
        ...
        <div class="form-group label-floating">
            <label class="control-label" >댓글</label>
	...
	<button type="submit" id="btn_submit" class="btn btn-raised btn-primary btn-sm">댓글등록</button>
	</div>
	</div>
    </form>
</aside>
