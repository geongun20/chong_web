...
<div id="bo_list">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col"><?php echo subject_sort_link('wr_1', $qstr2, 1) ?>진행상황 <i class="fas fa-sort"></i></a></th>
        ...
        for ($i=0; $i<count($list); $i++) {
	        if (!$is_admin) {
	        	$secret_name = preg_replace('/.(?=.$)/u','*',$list[$i]['wr_name']);
	        } else {
		        $secret_name = $list[$i]['wr_name'];
	        }
         ?>
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            ...
            <td class="td_name sv_use d-none d-sm-none d-md-table-cell"><?php echo $secret_name?></td>
            <td class="td_date d-none d-sm-none d-md-table-cell"><?php echo $list[$i]['datetime2'] ?></td>
            <td class="td_name">
	            <?php
	            if ($list[$i]['comment_cnt']) {
		            sql_fetch(" update $write_table set wr_1='답변 완료' where wr_id='{$list[$i]['wr_id']}' ");
					echo "답변 완료"; 
				} elseif ($list[$i]['wr_1']) {
					echo "확인 중";
	            	} else {
